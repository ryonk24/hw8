...
class Cli {
  ...
  createCar(): void {
    inquirer
      ...
      .then((answers) => {
        const vehicle: Car | Truck | Motorbike = new Car(
          Cli.generateVin(),
          answers.color,
          answers.make,
          answers.model,
  ...
  createTruck(): void {
    inquirer
      ...
      .then((answers) => {
        ...
        const vehicle: Car | Truck | Motorbike = new Truck(
          Cli.generateVin(),
          answers.color,
          answers.make,
          answers.model,
  ...
  createMotorbike(): void {
    inquirer
      ...
      .then((answers) => {
        ...
        const vehicle: Car | Truck | Motorbike = new Motorbike(
          Cli.generateVin(),
          answers.color,
          answers.make,
          answers.model,
          ...
          for (let i = 0; i < this.vehicles.length; i++) {
            if (this.vehicles[i].vin === this.selectedVehicleVin && this.vehicles[i] instanceof Truck) {
              inquirer
                ...
                .then((answers) => {
                  // use vin to find the selected vehicle
                  for (let i = 0; i < this.vehicles.length; i++) {
                    if (answers.selectedVehicleVin === this.vehicles[i].vin) {
                      (this.vehicles[i] as Truck).tow(this.vehicles[i]);
                    }
                  }
                })
            }
