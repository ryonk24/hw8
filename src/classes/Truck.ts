...
// import AbleToTow from '../interfaces/AbleToTow.js'; // Removed as it is unused
...
class Truck extends Vehicle {
  ...
  tow(vehicle: Truck | Motorbike | Car): void {
    ...
    const vehicleMakeAndModel = 'make' in vehicle && 'model' in vehicle ? `${vehicle.make} ${vehicle.model}` : 'unknown vehicle';
    if ('weight' in vehicle && vehicle.weight <= this.towingCapacity) {
      console.log(`Towing ${vehicleMakeAndModel}...`);
    }
    else {
      console.log(`The ${vehicleMakeAndModel} is too heavy to be towed.`);
    }
  }
